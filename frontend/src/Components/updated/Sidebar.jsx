...
const DraggableElement = ({ type, icon: Icon, label, onAdd }) => {
  ...
  return (
    <div ref={drag} className="element-item" style={{ opacity: isDragging ? 0.5 : 1 }} onClick={() => onAdd({ type })}>
      <Icon size={24} />
...
function Sidebar({
  ...
  useEffect(() => {
    ...
    const dateCol = allColumns.find(
      (col) => col.column_name.toLowerCase() === "date" || col.data_type.toLowerCase().includes("date"),
    )
    if (dateCol) {
  ...
  const fetchDatabases = async () => {
    try {
      ...
      setDatabases(response.data.map((db) => ({ datname: db.datname })))
    } catch (error) {
  ...
  const fetchTables = async (database) => {
    try {
      ...
      setTables(response.data.map((table) => table.table_name))
    } catch (error) {
  ...
  const fetchTableData = async () => {
    try {
      ...
      // Use the deployment URL instead of localhost
      ...
      if (response.data && Array.isArray(response.data)) {
        if (response.data.length > 0) {
          ...
          setErrorMessage("No data found. Please check your query parameters.")
        }
      } else {
      ...
      setIsNextDisabled(true)
      setErrorMessage(
        error.response?.data?.details || error.response?.data?.error || "An error occurred while fetching data",
      )

      // Clear table selection if table not found
      if (error.response?.data?.details?.includes("Table not found")) {
        setSelectedTable("")
      }
    }
  }

  const clearStateData = (currentStep) => {
    if (currentStep <= 2) {
      setJoinType("")
      setJoinTable("")
      setJoinCondition("")
          ...
          <div className="space-y-4">
            ...
            {selectedDatabase && (
              <div>
                ...
                {errorMessage && errorMessage.includes("Table not found") && (
    ...
    <div className="sidebar">
      ...
      <div className="element-list ">
        {filteredElements.map((element) => (
          <DraggableElement key={element.type} {...element} onAdd={onElementAdd} />
        ))}
      </div>
      <div className="data-selector p-4 mx-2 mt-2">{renderStep()}</div>
      <ErrorModal
        isVisible={isErrorModalVisible}
        onClose={handleCloseErrorModal}
        message={
          errorMessage || "Empty Data returned from the Server. Please try again switching the tables or constraints."
        }
      />
